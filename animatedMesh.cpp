...
//  std::cout << "Duration: " << sModel.m_Animation.Duration << "\n";
  if (MeshStored) {
    while (models.size()<=renderFrame) {
      ...
      for (int i = 0; i < indices.size(); i++) {
        int f = indices[i];
        // if (model1.pTransformedVertices[f].x > model2.max.x) {model2.max.x = model1.pTransformedVertices[f].x;}
        // if (model1.pTransformedVertices[f].y > model2.max.y) {model2.max.y = model1.pTransformedVertices[f].y;}
        // if (model1.pTransformedVertices[f].z > model2.max.z) {model2.max.z = model1.pTransformedVertices[f].z;}

        // if (model1.pTransformedVertices[f].x < model2.min.x) {model2.min.x = model1.pTransformedVertices[f].x;}
        // if (model1.pTransformedVertices[f].y < model2.min.y) {model2.min.y = model1.pTransformedVertices[f].y;}
        // if (model1.pTransformedVertices[f].z < model2.min.z) {model2.min.z = model1.pTransformedVertices[f].z;}
